...
上图是模型生成完成后的主界面。窗口采用“左侧参数面板 + 右侧三维视口”的工作布局，适合边修改参数边检查几何结果。

| 界面区域 | 内容与作用 |
| --- | --- |
| 顶部工具栏 | “导出模型”保存 STL/OBJ/PLY；“导出 COMSOL 流体网格”保存 BDF、MSH 和质量文件；“网格质量”查看体单元质量统计；“低质量单元”定位低缩放雅可比单元；“仿真区域”查看入口、出口和壁面分组；“重置视角”和“亮色背景”用于视口显示控制。 |
| 曲面定义 | 选择 Gyroid、Diamond、Primitive、I-WP、Neovius 或“自定义公式”。自定义公式模式会显示公式输入框并隐藏不适用的周期数量设置。 |
| 结构模式 | “片层结构”生成中性面两侧的实体壁，“实体结构”保留隐式函数的一侧；两种模式分别使用目标壁厚或等值面偏移控制结构。 |
| 外形尺寸 / 周期数量 | 设置 X、Y、Z 方向的模型尺寸和内置 TPMS 的周期数，模型中心作为坐标原点。 |
| 结构参数 | 设置壁厚、等值面偏移和每周期采样数。采样数越高，曲面细节越平滑，但生成时间和内存占用也会增加。 |
| 孔隙率控制 | 启用“按目标孔隙率自动求解”后，软件自动反求壁厚或等值面偏移，并在右侧统计区显示实际孔隙率和误差。 |
| 右侧统计区 | 显示三角面数、模型体积、相对密度和孔隙率；这些数值对应当前完整导出模型，而不是仅用于显示的简化预览。 |
| 三维视口 | 使用 GPU 绘制封闭模型。鼠标拖动旋转，滚轮缩放；亮色背景模式会使用深色模型和轮廓增强，提高模型与背景的对比度。 |
| 底部状态栏 | 显示生成进度、预览面数、导出面数、孤立碎片清理数量，以及质量分析或仿真区域预览的当前状态。 |

典型建模流程是：先在左侧设置曲面、尺寸和结构参数，点击“生成模型”；生成完成后在右侧拖动检查孔道和壁厚，再根据需要打开“网格质量”或“仿真区域”。工具栏的诊断按钮会随着模型状态变化：模型生成后可直接打开“网格质量”和“仿真区域”，而“低质量单元”需要先得到 CFD 体网格质量结果。

### COMSOL CFD 网格设置

![TPMS Studio CFD 网格设置界面](image-cfd.png)

下图展示左侧参数面板滚动到 CFD 区域后的状态。CFD 参数只影响流体域划分和质量分析，不会改变右侧已经生成的 TPMS 固体模型。
