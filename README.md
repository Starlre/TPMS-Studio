...
上图是模型生成完成后的主界面。窗口采用“左侧参数栏 + 右侧三维视口”的工程软件布局：参数始终位于固定操作区，视口保持独立，适合反复调整、生成并对照检查模型。
...
| 左侧参数栏 | 参数栏宽度可通过分隔线调整，采用“几何定义”“结构控制”“CFD 网格”三个页签；内容较多时只滚动左栏，不影响三维视口。 |
...
| 固定生成按钮 | “生成模型”固定在左栏底部，即使 CFD 参数发生滚动也始终可见。 |
| 模型统计条 | 显示三角面数、模型体积、相对密度和孔隙率；这些数值对应当前完整导出模型，而不是仅用于显示的简化预览。 |
...
上图展示切换到左侧“CFD 网格”页签后的状态。CFD 参数在左栏内纵向滚动，只影响流体域划分和质量分析，不会改变右侧已经生成的 TPMS 固体模型。
